    ...
    for set in sets:
        ...
        for problem in set.getProblems():   # Your agent will solve one problem at a time.
            ...
            if problem.getCorrect() == 'Correct':
                correct[set.getName()] += 1

            results.write("%s\n" % result)
        results.write("\n")
    for key in count:
        print '{}/{}'.format(correct[key], count[key]), key
    print '{}/{}'.format(sum(correct.values()), sum(count.values())), 'Total'
    print '{}/{}'.format(int(sum(correct.values()) * 1.65), sum(count.values()) * 2), 'Forecast'
