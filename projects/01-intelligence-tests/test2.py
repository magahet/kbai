...
def main():
    ...
    for file in os.listdir("dev-problems"): # One problem set per folder in /Problems/
        newSet = ProblemSet(file)       # Each problem set is named after the folder in /Problems/
        sets.append(newSet)
        for problem in os.listdir("dev-problems" + os.sep + file):  # Each file in the problem set folder becomes a problem in that set.
            f = open("dev-problems" + os.sep + file + os.sep + problem) # Make sure to add only problem files to subfolders of /Problems/
            newSet.addProblem(f)
